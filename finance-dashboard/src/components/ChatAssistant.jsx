  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 w-16 h-16 bg-brand-green text-slate-950 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(163,255,18,0.3)] z-[60] cursor-pointer group"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 w-[400px] h-[600px] glass-card overflow-hidden z-[70] flex flex-col border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
          >
            ...
            <div className="p-6 border-b border-white/5 bg-slate-900/40 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black text-white uppercase tracking-tight">VYNEX Inteligência</h3>
                  <div className="flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-green animate-pulse" />
                    <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Multi-Agentes Ativos</p>
                  </div>
                </div>
              </div>
              <button 
            ...
            <div 
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-950/20 no-scrollbar"
            >
              <div className="flex gap-3 max-w-[85%]">
                ...
                <div className="bg-slate-900 p-4 rounded-2xl rounded-tl-none border border-white/5 space-y-3">
                  <p className="text-xs text-slate-200 leading-relaxed">
                    Olá! Estou analisando seus dados agora. Identifiquei seu saldo consolidado e seu Score Vynex 2.0.
                  </p>
                  <p className="text-xs text-slate-200 leading-relaxed font-bold">
                    O que você quer resolver primeiro: seus gastos ou liberar limite de crédito?
                  </p>
                </div>
              </div>
                  ...
                  <div className={`flex gap-3 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`p-4 rounded-2xl border ${
                      msg.role === 'user' 
                        ...
                        : 'bg-slate-900 p-4 rounded-2xl rounded-tl-none border border-white/5 text-slate-200'
                    }`}>
            ...
            <div className="px-6 py-3 border-t border-white/5 bg-slate-900/20 flex gap-2 overflow-x-auto no-scrollbar">
              {[
                { icon: <TrendingUp size={12}/>, text: 'Ver meu score' },
                { icon: <ShieldCheck size={12}/>, text: 'Dicas de crédito' }
              ].map((btn, i) => (
                <button
                  key={i}
                  onClick={() => {
            ...
            <form onSubmit={handleSend} className="p-6 border-t border-white/5 bg-slate-900/40">
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-950 border border-white/10 rounded-2xl py-4 pl-4 pr-14 text-xs text-white focus:border-brand-green focus:outline-none transition-all placeholder:text-slate-700"
                />
                <button 
                  type="submit"
                  disabled={!input.trim() || isTyping}
                  className="absolute right-2 top-2 w-10 h-10 bg-brand-green text-slate-950 rounded-xl flex items-center justify-center disabled:opacity-30 transition-all hover:scale-105"
                >
                  <Send size={18} />
                </button>
              </div>
